...
- Google Chrome (нужен для Selenium‑fallback).
...
Вся конфигурация хранится в Settings (config.py) и читается из .env через Settings.from_env().

ВАЖНО про ключ API:
- Предпочтительный ключ в .env: YM_API_KEY
- Для совместимости поддерживается YMAPIKEY (будет прочитан, если YM_API_KEY не задан)
...
- MAX_FAXES: максимум факсов (по умолчанию 3)
...
- config.py — Settings и чтение .env
...
## ПЛАН РАЗВИТИЯ: BROWSER‑РЕЖИМ

Планируется добавить отдельный режим “BROWSER”, совмещающий Selenium‑браузер и консольный контроль, чтобы формировать Excel прямо из живой выдачи Яндекс.Карт.

Идея механизма:
- Запускается режим, автоматически открывается Яндекс.Карта в Chrome (через Selenium).
- Пользователь вручную выбирает запрос и область поиска через интерфейс карты (зум/перемещение).
- После нажатия Enter программа считывает организации из боковой панели и формирует Excel.
- Дополнительно программа автоматически пролистывает боковую панель до конца (включая блоки с контактами), чтобы убедиться, что список компаний полностью подгружен до выгрузки.
